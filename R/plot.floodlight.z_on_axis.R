    ...
    plot.floodlight.z_on_axis = function(xlab , floodlight , histogram , data ,  ylab1 , spotlights , cols , nux , nuz,  zs ,  fxz)
                                           #(fxz, gr, xlab, )
    ...
    #Set ylim
        ylim = range(floodlight.df[,c('conf.low','conf.high')])     #Default y-range from smallest to larger CI
    ...
    #Empty plot
        plot(zs , floodlight[[1]]$estimate , type='n' , xlab=xlab , ylab=ylab1 , las=1 , ylim=ylim , xlim=xlim , yaxt='n',cex.lab=1.3)
    ...
    #Loop the 2 or 3 values of x slopes
        for (j in 1:nux) {
              ...
              g=rep(g,length(zs))
            #Lines
              line.seg(zs,floodlight[[j]]$estimate,
                       lwd=4*gr[,j], 
                       col=cols[j],
                       g=g,
                       lty=ltys[j]) 
            ...
            #Changing both width and tly leads to weird looking lines
               ...
               # which we detect with as many bins of z as there are unique z-values
                  if (nuz == n.zbins) points(zs,floodlight[[j]]$estimate, col=adjustcolor2(cols[j],g),pch=16) 
