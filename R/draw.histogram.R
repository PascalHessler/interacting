    ...
    draw.histogram = function(fxz.list, focal, moderation, xs, zs, ux, nux, cols,ylim,xlim)
          {
          ...
          #Determine # of lines and bins
            if (focal %in% c('continuous','discrete')) {
    ...
    if (focal=='discrete')
      {
        #Adjust y coordinates to be bottom of figure
          ...
          #Add sample size values
            text(xs ,y1,rowSums(fxz),pos=3,cex=.8,font=3,col='gray38')
       }
