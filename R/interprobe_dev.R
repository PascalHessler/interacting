...
#9 PLOT SIMPLE SLOPES
       if (draw.simple.slopes==TRUE)
       {
        ...
        if (nux %in% c(2,3)) 
          plot.simple.slopes.z_on_axis (xlab, simple.slopes, histogram, data,xs, ylab1,gr,spotlights,cols,nux,zs,bins,fx,nbins)
        }
       ...
       #9.2 x has 4+
        ...
        # TEMP - single width line
          gr=list()
          for (j in 1:3) gr[[j]] =rep(1,100)
          xlab='x: focal predictor'
        #  </temp>
          
          plot.simple.slopes.x_on_axis  (xlab, simple.slopes, histogram, data,xs, ylab1 ,gr,spotlights,cols)
